...
flow_daily_temp <- right_join(flow_all_days, flow_obs, by="date") #[335,1453]
dim(flow_daily_temp)
flow_daily <- flow_daily_temp[,-which((colnames(flow_daily_temp)=="bacteria" | 
                                                   colnames(flow_daily_temp)=="discharge"))] #[336,2215]
#dim(flow_daily)
# then reduce daily simulated observations to weekly averages for each of the sims #[336,2215]
#head(colnames(flow_daily)) #  date 
nsim_cols <- ncol(flow_daily) #2215 date + sims field
flow_daily_data <- as.xts(flow_daily[2:nsim_cols],order.by=as.Date(flow_daily$date)) #[336,2214]
flow_daily <- flow_daily[,-1] #[336,2215]
flow_weekly <- as.data.frame(apply.weekly(flow_daily_data,mean)) #[203,1450]
flow_weekly<-flow_weekly[,c(1:1000)]
dim(flow_weekly)
...
flow_obs_weekly<-cbind(date, flow_obs_weekly)
flow_obs_weekly
...
ggplot() +
  ...
  geom_point(data =flow_obs_weekly, aes(x=date, y=log10(discharge), colour = "discharge"))+
  scale_colour_manual("", 
                      breaks = c("first_q",   "four_q",  "sev_q",  "discharge"),
                      values =c("grey70", "#CC79A7", "grey30",  "darkblue")) +
  xlab(" ") +
...
ggplot(data, aes(x=target, y=mnse, fill=interval)) +
  geom_boxplot()+
